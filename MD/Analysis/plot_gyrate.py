    ...
    with open(filename, "r") as f:
        for line in f:
            if not line.startswith(("#", "@")):
                split = line.split()
                data.append([float(split[i]) + start if i == 0 else float(split[i]) for i in range(2)])
    return np.array(data)

def plot_running_avg(x, y, c="red", labelname="1-ns running avg."):
    window_size = 500
...
for protein in ["qty", "wt"]:
    ...
    x = data[:, 0]
    y = data[:, 1]
    plt.plot(x, y, color="black", linewidth=0.5, label="Radius of gyration")
